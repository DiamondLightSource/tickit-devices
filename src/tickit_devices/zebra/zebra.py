...
from . import AndOrBlockConfig
from ._common import param_types, register_names, mux_types, Block
...
class ZebraDevice(SystemSimulationComponent):
    ...
    def __init__(self, name: ComponentID, host: str = "localhost", port: int = 7012):
        ...
        self.blocks = {
            block.name: block for block in [
                AndOrBlockConfig(ComponentID(f"{ANDOR}{NUM + 1}"), self.params)
                for ANDOR in {"AND", "OR"} for NUM in range(4)
            ]
        }
...
class ZebraAdapter(ComposedAdapter[bytes, ZebraDevice]):
    ...
    @RegexCommand(br"W([0-9A-F]{2})([0-9A-F]{4})\n", interrupt=True)
